...
// ==========================================
// ระบบค้นหา & แบ่งหน้า (Search & Pagination) - Companies
// ==========================================
$search_company = isset($_GET['search_company']) ? $conn->real_escape_string($_GET['search_company']) : '';
$where_comp_sql = "";
if ($search_company != '') {
    $where_comp_sql = "WHERE company_name LIKE '%$search_company%' OR address LIKE '%$search_company%'";
}

$limit_comp = 20; // 20 รายการต่อหน้า
$page_comp = isset($_GET['page_comp']) ? (int) $_GET['page_comp'] : 1;
if ($page_comp < 1) $page_comp = 1;
$offset_comp = ($page_comp - 1) * $limit_comp;

$count_comp_sql = "SELECT COUNT(company_id) as total FROM companies $where_comp_sql";
$count_comp_result = $conn->query($count_comp_sql);
$total_comp_rows = $count_comp_result->fetch_assoc()['total'];
$total_comp_pages = ceil($total_comp_rows / $limit_comp);
...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-custom sticky-top py-3">
        <div class="container">
            <a class="navbar-brand fw-bold" href="#"><i class="fas fa-wand-magic-sparkles text-warning"></i> Admin Panel</a>
            <div class="d-flex align-items-center">
                <span class="me-3 text-muted" style="font-weight: 500;">👋 สวัสดี, คุณ <?php echo htmlspecialchars($admin_fullname); ?></span>
                <a href="logout.php" class="btn btn-rounded btn-pastel-danger btn-sm"><i class="fas fa-arrow-right-from-bracket"></i> ออกจากระบบ</a>
            </div>
        </div>
    </nav>
    ...
    <div class="container mt-5 pb-5">
        ...
        <div class="main-card">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
                ...
                <a href="admin_dashboard.php?action=clear_all_sessions" class="btn btn-rounded btn-pastel-danger py-2 px-4" onclick="return confirm('⚠️ คำเตือนน่ารักๆ: คุณแน่ใจนะว่าจะเตะทุกคนออกจากระบบ (ทั้ง Web และ App)? (ยกเว้นตัวคุณเอง)');">
            ...
            <ul class="nav nav-pills mb-4" id="adminTabs">
                ...
                <li class="nav-item ms-lg-2 mt-2 mt-lg-0">
                    <a class="nav-link bg-white border border-primary text-primary shadow-sm" href="admin_internships.php">
                        <i class="fas fa-user-check me-1"></i> จัดการสถานะฝึกงาน <i class="fas fa-arrow-right ms-1" style="font-size: 0.8em;"></i>
                    </a>
                </li>
                <li class="nav-item ms-lg-2 mt-2 mt-lg-0">
                    <a class="nav-link bg-white border border-success text-success shadow-sm" href="admin_supervision.php">
                        <i class="fas fa-calendar-check me-1"></i> กำหนดการนิเทศ <i class="fas fa-arrow-right ms-1" style="font-size: 0.8em;"></i>
                    </a>
                </li>
            </ul>
            ...
            <div class="tab-content">
                <div class="tab-pane fade <?php echo $active_tab == 'users' ? 'show active' : ''; ?>" id="users">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <p class="text-muted mb-0"><i class="fas fa-info-circle text-primary"></i> พบข้อมูลผู้ใช้งานทั้งหมด <strong class="text-primary"><?php echo $total_rows; ?></strong> รายการ</p>
                        <div>
                            <button class="btn btn-pastel-primary btn-rounded shadow-sm px-4 me-2" style="padding: 8px 25px; font-size: 1rem;" data-bs-toggle="modal" data-bs-target="#addPersonnelModal">
                                <i class="fas fa-user-plus me-1"></i> เพิ่มบุคลากร
                            </button>
                            <button class="btn btn-upload btn-rounded shadow-sm px-4" style="padding: 8px 25px; font-size: 1rem;" data-bs-toggle="modal" data-bs-target="#importCsvModal">
                                <i class="fas fa-file-csv me-1"></i> นำเข้ารายชื่อนักศึกษา
                            </button>
                        </div>
                    </div>

                    <form method="GET" action="admin_dashboard.php" class="row g-3 mb-4 bg-light p-3 rounded-4 shadow-sm align-items-end">
                        <input type="hidden" name="tab" value="users">
                        <div class="col-md-3">
                            <label class="form-label text-muted small fw-bold ms-2">ค้นหาชื่อ / รหัส 🔍</label>
                            <input type="text" name="search_text" class="form-control border-0 shadow-sm" placeholder="พิมพ์ชื่อหรือรหัส..." value="<?php echo htmlspecialchars($search_text); ?>">
                        </div>
                        <div class="col-md-3">
                            ...
                            <select name="search_level" class="form-select border-0 shadow-sm">
                                ...
                                <option value="s" <?php if ($search_level == 's') echo 'selected'; ?>>🎓 นักศึกษา</option>
                                <option value="t" <?php if ($search_level == 't') echo 'selected'; ?>>👨‍🏫 อาจารย์</option>
                                <option value="a" <?php if ($search_level == 'a') echo 'selected'; ?>>👑 ผู้ดูแลระบบ</option>
                            </select>
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-3 d-flex gap-2">
                            <button type="submit" class="btn btn-pastel-primary btn-rounded flex-grow-1 shadow-sm"><i class="fas fa-search"></i> ค้นหา</button>
                            <a href="admin_dashboard.php?tab=users" class="btn btn-light btn-rounded border text-muted shadow-sm"><i class="fas fa-undo"></i> ล้างค่า</a>
                        </div>
                    </form>
                        ...
                        <nav aria-label="Page navigation" class="mt-4 pt-3 border-top">
                            <ul class="pagination justify-content-center mb-0">
                                <li class="page-item <?php echo ($page <= 1) ? 'disabled' : ''; ?>">
                                    <a class="page-link shadow-sm bg-white" href="?page=<?php echo $page - 1 . $query_string; ?>"><i class="fas fa-chevron-left"></i> ก่อนหน้า</a>
                                </li>
                                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                                    <li class="page-item <?php echo ($page == $i) ? 'active' : ''; ?>">
                                        <a class="page-link shadow-sm bg-white" href="?page=<?php echo $i . $query_string; ?>"><?php echo $i; ?></a>
                                    </li>
                                <?php endfor; ?>
                                <li class="page-item <?php echo ($page >= $total_pages) ? 'disabled' : ''; ?>">
                                    <a class="page-link shadow-sm bg-white" href="?page=<?php echo $page + 1 . $query_string; ?>">ถัดไป <i class="fas fa-chevron-right"></i></a>
                                </li>
                            </ul>
                        </nav>
                    <?php endif; ?>
                ...
                <div class="tab-pane fade <?php echo $active_tab == 'companies' ? 'show active' : ''; ?>" id="companies">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <p class="text-muted mb-0"><i class="fas fa-info-circle text-primary"></i> ข้อมูลบริษัทจะถูกแสดงให้นักศึกษาเลือกในขั้นตอนยื่นฝึกงาน</p>
                        <button class="btn btn-pastel-primary btn-rounded shadow-sm" data-bs-toggle="modal" data-bs-target="#addCompanyModal">
                    ...
                    <form method="GET" action="admin_dashboard.php" class="row g-3 mb-4 bg-light p-3 rounded-4 shadow-sm align-items-end">
                        ...
                        <div class="col-md-8">
                            <label class="form-label text-muted small fw-bold ms-2">ค้นหาชื่อบริษัท / ที่อยู่ 🔍</label>
                            <input type="text" name="search_company" class="form-control border-0 shadow-sm" placeholder="พิมพ์ชื่อบริษัท หรือ ที่อยู่..." value="<?php echo htmlspecialchars($search_company); ?>">
                        </div>
                        <div class="col-md-4 d-flex gap-2">
                            <button type="submit" class="btn btn-pastel-primary btn-rounded flex-grow-1 shadow-sm"><i class="fas fa-search"></i> ค้นหา</button>
                            <a href="admin_dashboard.php?tab=companies" class="btn btn-light btn-rounded border text-muted shadow-sm"><i class="fas fa-undo"></i> ล้างค่า</a>
                        </div>
                    </form>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                <?php
                                $comp_sql = "SELECT * FROM companies $where_comp_sql ORDER BY company_id DESC LIMIT $limit_comp OFFSET $offset_comp";
                                $comp_res = $conn->query($comp_sql);

                                if ($comp_res && $comp_res->num_rows > 0) {
                                    $seq_id = $offset_comp + 1;
                                    while ($c = $comp_res->fetch_assoc()) {
                                        echo "<tr>";
                                        echo "<td class='ps-4 fw-bold text-muted'>" . $seq_id++ . "</td>";
                    ...
                    <?php if ($total_comp_pages > 1):
                        $query_comp_string = "&search_company=" . urlencode($search_company) . "&tab=companies";
                        ?>
                        <nav aria-label="Page navigation" class="mt-4 pt-3 border-top">
                            <ul class="pagination justify-content-center mb-0">
                                <li class="page-item <?php echo ($page_comp <= 1) ? 'disabled' : ''; ?>">
                                    <a class="page-link shadow-sm bg-white" href="?page_comp=<?php echo $page_comp - 1 . $query_comp_string; ?>"><i class="fas fa-chevron-left"></i> ก่อนหน้า</a>
                                </li>
                                <?php for ($i = 1; $i <= $total_comp_pages; $i++): ?>
                                    <li class="page-item <?php echo ($page_comp == $i) ? 'active' : ''; ?>">
                                        <a class="page-link shadow-sm bg-white" href="?page_comp=<?php echo $i . $query_comp_string; ?>"><?php echo $i; ?></a>
                                    </li>
                                <?php endfor; ?>
                                <li class="page-item <?php echo ($page_comp >= $total_comp_pages) ? 'disabled' : ''; ?>">
                                    <a class="page-link shadow-sm bg-white" href="?page_comp=<?php echo $page_comp + 1 . $query_comp_string; ?>">ถัดไป <i class="fas fa-chevron-right"></i></a>
                                </li>
                            </ul>
                        </nav>
                    <?php endif; ?>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content shadow-lg border-0" style="border-radius: 25px;">
                <div class="modal-header border-0 pb-0 mt-3 px-4">
                    <h4 class="modal-title fw-bold text-dark"><i class="fas fa-file-csv text-success me-2"></i> นำเข้ารายชื่อนักศึกษา (CSV)</h4>
                ...
                <div class="modal-body px-4 pt-2 pb-4">
                    ...
                    <div class="instruction-box mb-4 shadow-sm">
                        <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                            <h6 class="fw-bold mb-0 text-dark"><i class="fas fa-list-ol text-warning me-2"></i> วิธีการใช้งานและคำแนะนำ</h6>
                            <a href="admin_dashboard.php?action=download_template" class="download-btn shadow-sm text-decoration-none" style="background-color: #e6f4ea; color: #1e8e3e; border-color: #cce8d6;">
                        ...
                        <ol class="mb-0 small" style="line-height: 1.8;">
                            ...
                            <li><b>กรอกข้อมูลนักศึกษา:</b> โดยให้ <span class="text-danger fw-bold">คอลัมน์ A คือ "รหัสนักศึกษา"</span> และ <span class="text-primary fw-bold">คอลัมน์ B คือ "ชื่อ-นามสกุล"</span></li>
                            <li class="p-2 mt-2 mb-2 bg-white rounded border border-warning">
                                <b class="text-danger"><i class="fas fa-exclamation-triangle"></i> สำคัญมาก! ขั้นตอนการบันทึกไฟล์:</b><br>
                                เมื่อกรอกข้อมูลใน Excel เสร็จแล้ว ให้ไปที่ <b>File > Save As (บันทึกเป็น)</b> <br>
                                แล้วเปลี่ยนช่อง <i>Save as type</i> ให้เป็น <b class="text-danger text-decoration-underline">CSV UTF-8 (Comma delimited) (*.csv)</b> เท่านั้น
                            </li>
                            <li><b>อัปโหลดไฟล์:</b> นำไฟล์นามสกุล <b>.csv</b> ที่ได้จากข้อ 3 มาลากวางหรืออัปโหลดที่ช่องด้านล่าง</li>
                            <li><b>ระบบจะตรวจสอบและเพิ่มข้อมูล:</b> รหัสผ่านเริ่มต้นจะถูกตั้งเป็น "รหัสนักศึกษา" ให้อัตโนมัติ (ข้ามรายชื่อที่ซ้ำ)</li>
                        </ol>
                    </div>
                    ...
                    <form action="admin_dashboard.php" method="POST" enctype="multipart/form-data">
                        ...
                        <div id="file-name-display" class="text-center fw-bold text-success mb-4 d-none p-2 bg-light rounded-pill border">
                            <i class="fas fa-file-alt me-2"></i> ไฟล์ที่เลือก: <span id="file-name-text"></span>
                        </div>

                        <div class="text-center mt-4">
                            <button type="button" class="btn btn-rounded btn-light border px-4 me-2" data-bs-dismiss="modal">ยกเลิก</button>
                            <button type="submit" class="btn btn-upload shadow"><i class="fas fa-save me-2"></i> อัปโหลดและเพิ่มรายชื่อเข้าสู่ระบบ</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content shadow-lg">
                <div class="modal-header border-0 pb-0" style="background-color: var(--primary-soft);">
                    <h5 class="modal-title fw-bold text-white"><i class="fas fa-user-pen me-2"></i>แก้ไขข้อมูลสมาชิก</h5>
                    ...
                    <form action="admin_dashboard.php" method="POST" id="editUserForm">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="academic_year" id="modal_year" class="form-control text-center" required>
                            </div>
                            <div class="col-md-6 mb-3">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-muted ms-2">กำหนดรหัสผ่านใหม่ 🔑 <span class="text-danger fw-normal" style="font-size: 0.85em;">(ปล่อยว่างไว้หากไม่ต้องการเปลี่ยน)</span></label>
                            <input type="password" name="new_password" id="modal_new_password" class="form-control" placeholder="พิมพ์รหัสผ่านใหม่ที่ต้องการ...">
                        </div>
                    </form>
                    <hr class="my-4">
                    <form action="admin_dashboard.php" method="POST" onsubmit="return confirm('แน่ใจหรือไม่ที่จะรีเซ็ตรหัสผ่าน? \n(รหัสผ่านจะถูกตั้งให้เป็นเลข ID ของผู้ใช้นี้)');">
                        ...
                        <div class="d-flex justify-content-between align-items-center bg-light p-3 rounded-4 border">
                            ...
                            <button type="submit" class="btn btn-outline-danger btn-sm btn-rounded"><i class="fas fa-undo"></i> รีเซ็ตเลย</button>
                        </div>
                    </form>
                </div>
                <div class="modal-footer border-0 pt-0 pb-4 px-4 mt-3">
                    ...
                    <button type="button" class="btn btn-rounded btn-pastel-primary px-4" onclick="document.getElementById('editUserForm').submit();">💾 บันทึกการแก้ไข</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content shadow-lg">
                <form action="admin_dashboard.php" method="POST">
                    ...
                    <div class="modal-header border-0 pb-0" style="background-color: var(--primary-soft);">
                        <h5 class="modal-title fw-bold text-white"><i class="fas fa-building me-2"></i>เพิ่มข้อมูลบริษัทใหม่</h5>
                        <button type="button" class="btn-close btn-close-white me-2 mt-2" data-bs-dismiss="modal" style="filter: brightness(0) invert(1);"></button>
                    </div>
                    <div class="modal-body pt-4 px-4">
                        <div class="mb-3">
                        ...
                        <div class="row mb-2">
                            <div class="col-md-6 mb-2">
                                ...
                                <input type="text" name="latitude" id="add_lat" class="form-control" placeholder="เช่น 13.7563">
                            </div>
                            <div class="col-md-6 mb-2">
                                <label class="form-label fw-bold small text-muted ms-2">ลองจิจูด (Longitude)</label>
                                <input type="text" name="longitude" id="add_lng" class="form-control" placeholder="เช่น 100.5018">
                            </div>
                        </div>

                        <div class="col-12 mb-3">
                            <button type="button" class="btn btn-sm btn-outline-primary w-100 rounded-pill mb-2" onclick="showMap('add')">
                                <i class="fas fa-map-marked-alt"></i> แสดง/อัปเดตแผนที่จากพิกัด
                            </button>
                            <div id="map_add" style="height: 250px; width: 100%; border-radius: 15px; display: none; border: 1px solid #ced4da;"></div>
                            <small class="text-muted d-none mt-1 ms-2" id="map_add_help">* สามารถเลื่อนหมุดบนแผนที่ หรือคลิกเพื่อเปลี่ยนพิกัดได้</small>
                        </div>
                    </div>
            ...
            <div class="modal-content shadow-lg">
                <form action="admin_dashboard.php" method="POST">
                    ...
                    <div class="modal-body pt-4 px-4">
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-muted ms-2">ชื่อบริษัท / สถานประกอบการ</label>
                            <input type="text" name="company_name" id="modal_company_name" class="form-control" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-muted ms-2">ที่อยู่แบบเต็ม</label>
                            <textarea name="address" id="modal_company_address" class="form-control" rows="2" required></textarea>
                        </div>
                        <div class="row mb-2">
                            <div class="col-md-6 mb-2">
                                <label class="form-label fw-bold small text-muted ms-2">ละติจูด (Latitude)</label>
                        ...
                        <div class="col-12 mb-3">
                            <button type="button" class="btn btn-sm btn-outline-primary w-100 rounded-pill mb-2" onclick="showMap('edit')">
                                <i class="fas fa-map-marked-alt"></i> แสดง/อัปเดตแผนที่จากพิกัด
                            </button>
                            <div id="map_edit" style="height: 250px; width: 100%; border-radius: 15px; display: none; border: 1px solid #ced4da;"></div>
                            <small class="text-muted d-none mt-1 ms-2" id="map_edit_help">* สามารถเลื่อนหมุดบนแผนที่ หรือคลิกเพื่อเปลี่ยนพิกัดได้</small>
                        </div>
                    </div>
                    <div class="modal-footer border-0 pt-0 pb-4 px-4">
                        <button type="button" class="btn btn-rounded btn-light border px-4" data-bs-dismiss="modal">ยกเลิก</button>
            ...
            <div class="modal-content shadow-lg border-0" style="border-radius: 25px;">
                <form action="admin_dashboard.php" method="POST">
                    ...
                    <div class="modal-header border-0 pb-0 mt-2 px-4">
                        <h5 class="modal-title fw-bold" style="color: #0d6efd;"><i class="fas fa-user-tie me-2"></i>เพิ่มบุคลากรใหม่</h5>
                        <button type="button" class="btn-close mt-1 me-1" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body pt-4 px-4 pb-2">
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-muted ms-2">รหัสประจำตัว (ID) <span class="text-danger">*</span></label>
                            <input type="text" name="users_name" class="form-control" required placeholder="เช่น T001, ADMIN01">
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-muted ms-2">ชื่อ-นามสกุล <span class="text-danger">*</span></label>
                            <input type="text" name="full_name" class="form-control" required placeholder="เช่น ดร.สมชาย ใจดี">
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-bold small text-muted ms-2">บทบาท <span class="text-danger">*</span></label>
                            ...
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-bold small text-muted ms-2">ปีการศึกษา <small>(ถ้ามี)</small></label>
                                <input type="text" name="academic_year" class="form-control text-center" value="-">
                            </div>
                        </div>
                        <div class="mb-3 p-3 bg-light rounded-4 border">
                            <label class="form-label fw-bold small text-dark ms-2 mb-1"><i class="fas fa-key text-warning"></i> ตั้งรหัสผ่านเริ่มต้น</label><br>
                            <small class="text-muted ms-2 d-block mb-2">หากปล่อยว่างไว้ ระบบจะใช้ <b>"รหัสประจำตัว (ID)"</b> เป็นรหัสผ่าน</small>
                            <input type="password" name="password" class="form-control" placeholder="พิมพ์รหัสผ่าน (ไม่บังคับ)">
                        </div>
                    </div>
                    <div class="modal-footer border-0 pt-0 pb-4 px-4 mt-2">
                        <button type="button" class="btn btn-rounded btn-light border px-4" data-bs-dismiss="modal">ยกเลิก</button>
                        <button type="submit" class="btn btn-rounded btn-pastel-primary px-4 shadow-sm"><i class="fas fa-save me-1"></i> บันทึกข้อมูล</button>
                    </div>
                </form>
            </div>
        </div>
