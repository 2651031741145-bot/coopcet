...
import 'dart:convert';
// 🚨 1. แก้ไข Import ให้ดึงหน้ารายละเอียดของนักศึกษามาแทน
import 'student_archive_detail_screen.dart'; 

class StudentArchiveScreen extends StatefulWidget {
  const StudentArchiveScreen({Key? key}) : super(key: key);
                        ...
                        rows: _students.map((s) {
                          return DataRow(
                            onSelectChanged: (selected) {
                              if (selected != null) {
                                // 🚨 2. เปลี่ยนให้เด้งไปที่หน้า StudentArchiveDetailScreen ที่ไม่มีคะแนนประเมิน
                                Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                        builder: (context) =>
                                            StudentArchiveDetailScreen(
                                                studentData: s)));
                              }
                            },
                            cells: [
